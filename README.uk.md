...
Agent Game Forge (**AGF**) — це десктопна IDE з відкритим кодом, яка дозволяє ШІ-агенту з кодування зібрати для вас повноцінні 2D-ігри — спрайти, parallax-фони, фізику, небезпеки, предмети, розташування сцен — і надає візуальний редактор, щоб перетягуванням підправити те, що агент зробив не так. **Ви обираєте агента** (Codex CLI або Claude Code) і **ви обираєте image gen** — принесіть свій API-ключ, або використовуйте вбудований image gen у Codex CLI (GPT-Image2). Сьогодні типовий вихідний формат — vanilla JS + Canvas (нульова прив'язка до фреймворку, запускається у будь-якому браузері); таргети рушіїв Godot 4 та Unity — у roadmap.
...
- 🖼️ **Принесіть свій image gen** — надайте API-ключ для бажаного провайдера зображень, або використовуйте вбудований image gen у Codex CLI (GPT-Image2). Ключі залишаються на вашій машині.
...
Ліцензовано під [Apache License, Version 2.0](LICENSE). Ви можете вільно використовувати, змінювати, форкати та поширювати — комерційно чи ні. Просто зберігайте повідомлення про copyright і ліцензію.
