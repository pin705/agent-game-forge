...
> 곧 공개: 프롬프트 → 플레이 가능한 플랫포머 → 라이브 편집 → CLI 전환을 보여주는 90초 데모 영상.

**히어로 샷** (AGF 창):

> _히어로 스크린샷을 사용할 수 있게 되면 여기에 삽입_

**Settings — BYOA 증명**:

> _에이전트 선택기 + API 키 + 이미지 생성 기본값을 보여주는 Settings 모달 스크린샷 삽입_

**씬 에디터 — 위험요소를 드래그하고 Play에서 확인**:
