function drawHud(ctx) {
  ...
  crispText(ctx, t("hp", { hp: p.hp, max: p.maxHp }), 32, 37, "bold 11px system-ui, sans-serif", "#fff", "left");
  var xpPct = Math.min(1, (p.xp || 0) / xpForNextLevel());
  gradientBar(ctx, 28, 46, 200, 9, xpPct, "#3ac47a", "#a8ff6a", "rgba(0,0,0,0.5)");
  crispText(ctx, t("level", { n: p.level || 1 }), 28, 80, "bold 14px system-ui, sans-serif", COLORS.gold, "left");
  crispText(ctx, t("kills", { n: state.killCount }), 120, 80, "13px system-ui, sans-serif", COLORS.text, "left");
...
function drawLevelUpOverlay(ctx) {
  ...
  crispText(ctx, t("levelUp"), 0, 0, "bold 34px system-ui, sans-serif", COLORS.gold, "center");
  ctx.restore();
  ...
  for (var i = 0; i < cards.length; i++) {
    ...
    crispText(ctx, t(cards[i].nameKey), cx + 75, cy + 40, "bold 16px system-ui, sans-serif", COLORS.text, "center");
    crispText(ctx, t(cards[i].descKey), cx + 75, cy + 70, "13px system-ui, sans-serif", COLORS.muted, "center");
    crispText(ctx, String(i + 1), cx + 75, cy + 98, "bold 13px system-ui, sans-serif", COLORS.gold, "center");
  }
  crispText(ctx, t("pickCard"), VIEW.w / 2, 404, "14px system-ui, sans-serif", COLORS.muted, "center");
}
