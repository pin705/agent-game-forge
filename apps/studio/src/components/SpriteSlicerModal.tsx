...
export function SpriteSlicerModal(props: SpriteSlicerModalProps) {
  ...
  useEffect(() => {
    ...
    imageDataUrl(props.projectPath, props.imageRelPath)
      .then((url) => {
        if (cancelled) return;
        if (!url) setImgError('Not an image (or could not be read)');
        else setImgUrl(url);
      })
      .catch((e) => {
  ...
  async function save(): Promise<boolean> {
    ...
    try {
      ...
      toast.error(
        `Could not save slicing: ${err instanceof Error ? err.message : String(err)}`,
      );
      return false;
    } finally {
      setSaving(false);
      ...
      <DialogContent className="max-w-4xl">
        ...
        <div className="grid gap-4 md:grid-cols-[1fr_280px]">
          ...
          <div className="space-y-4">
            <div className="space-y-3">
              <Field label="Columns" value={cols} min={1} max={16} onChange={setCols} />
              <Field label="Rows" value={rows} min={1} max={16} onChange={setRows} />
              <div className="grid grid-cols-2 gap-3 text-xs">
                <div className="rounded-md border px-2 py-1.5">
                  <div className="text-muted-foreground">Frame W</div>
                  <div className="font-mono font-medium">{Math.round(fW)}px</div>
                </div>
                <div className="rounded-md border px-2 py-1.5">
                  <div className="text-muted-foreground">Frame H</div>
            ...
            <div className="space-y-3">
              <Field label="Padding" value={pad} min={0} max={16} onChange={setPad} />
              <Field label="Offset X" value={offX} min={-32} max={32} onChange={setOffX} />
              <Field label="Offset Y" value={offY} min={-32} max={32} onChange={setOffY} />
            </div>
            ...
            <div className="space-y-1.5">
              <div className="text-xs text-muted-foreground">Animation preview</div>
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  <Field label="FPS" value={fps} min={1} max={24} onChange={setFps} />
        ...
        <DialogFooter className="items-center gap-2 sm:justify-between">
          <span className="text-xs text-muted-foreground">
            ...
            {savedAt && <span className="ml-2 text-success">✓ saved</span>}
          </span>
          <div className="flex gap-2">
            ...
            {props.onAskAgent && (
              <Button
                size="sm"
                disabled={saving || !naturalW}
                title="Save and ask the agent to apply this slicing to the engine config"
                onClick={async () => {
                  const ok = await save();
                  if (!ok) return;
