...
**Runtime smoke test (dynamic — catches what static can't).** After the static verifier passes, run `node .agents/tools/verify-runtime.mjs . 240`. It headlessly boots the game (stubbed Canvas/Image/Audio/DOM), simulates play (movement + actions) for N frames, and reports the FIRST runtime error with a stack — the dynamic complement that catches "boots then crashes / freezes mid-play" bugs the static checks miss. For DOM-panel genres (menus/dialogue via innerHTML), prefix `NOACT=1` to skip synthetic UI clicks. This is the chassis-correct revival of OpenGame's play-the-game debug loop.
