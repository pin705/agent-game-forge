...
// (message + top stack) instead of silently freezing. (Copied from side-scroll seed.)
...
function frame(nowMs) {
  try {
    ...
    // so the freeze reads as impact weight, not a stutter. (juice.js)
    ...
    updateParticles(dt);
    updateJuice(dt);
    updateStars(state.mode === "playing" ? sdt : dt);
    renderFrame();
    drawJuice(dom.ctx);
    requestAnimationFrame(frame);
  } catch (err) {
...
function updateScene(dt) {
  ...
  state.scroll += (cfg("stage").scrollSpeed ?? 80) * dt;
  updateShip(dt);
  updateWaves(dt);
  updateBullets(dt);
  resolveBulletHits();
...
function handleGlobalInput() {
  ...
  if (state.mode === "paused" && wasPressed("pause")) { state.mode = "playing"; }
}
