...
function handleMapClick(mx, my) {
  ...
  if (spot) placeTower(spot.id, spot.x, spot.y);
}

function wireMouseInput() {
  dom.canvas.addEventListener("click", function(e) {
    var rect = dom.canvas.getBoundingClientRect();
    var mx = (e.clientX - rect.left) * (VIEW.w / rect.width);
    var my = (e.clientY - rect.top) * (VIEW.h / rect.height);
    handleMapClick(mx, my);
  });
  dom.canvas.addEventListener("mousemove", function(e) {
    var rect = dom.canvas.getBoundingClientRect();
    state.cursor.x = (e.clientX - rect.left) * (VIEW.w / rect.width);
    state.cursor.y = (e.clientY - rect.top) * (VIEW.h / rect.height);
  });
}
...
function handleGlobalInput() {
  ...
  if ((state.mode === "gameover" || state.mode === "win") && wasPressed("start")) { startNewRun(); return; }
