...
### Multi-action entity (player, enemies, bosses, NPCs) — animations object

ANY entity with multiple actions MUST use the \`animations: { ... }\` open
object — one key per action, NOT a single \`sprite\` field. Each animation
references its OWN sheet generated by a separate \`generate2dsprite\` call.
...
[
  {
    ...
    "kind": "melee",
    "stats": { "hp": 2, "speed": 80, "damage": 1, "score": 100 },
    "animations": {
      "idle":   { "sprite": "assets/sprites/ashigaru_spearman/idle/sheet.png",   "frameW": 128, "frameH": 128, "fps": 6 },
      "walk":   { "sprite": "assets/sprites/ashigaru_spearman/walk/sheet.png",   "frameW": 128, "frameH": 128, "fps": 8 },
      "attack": { "sprite": "assets/sprites/ashigaru_spearman/attack/sheet.png", "frameW": 128, "frameH": 128, "fps": 10 }
    }
  },
  {
    "id": "yumi_archer",
    "kind": "ranged",
    "stats": { "hp": 2, "speed": 42, "damage": 1, "attackRange": 440, "projectile": "arrow_projectile" },
    "animations": {
      "idle":   { "sprite": "assets/sprites/yumi_archer/idle/sheet.png",   "frameW": 128, "frameH": 128, "fps": 6 },
      "attack": { "sprite": "assets/sprites/yumi_archer/attack/sheet.png", "frameW": 128, "frameH": 128, "fps": 8 }
    }
  }
]
\`\`\`

Notice: archer has NO walk (stationary), spearman has NO jump (ground melee).
**Animation set is per-entity, picked by role**, not a uniform list.

### Forbidden: combined-action sheet

\`\`\`json
// ❌ DON'T do this — collapses idle+attack into one 8-frame "combat"
{
  "id": "spearman",
  "sprite": "assets/sprites/spearman/sheet.png",   // ← single combined sheet
  "frameW": 128, "frameH": 128
  // runtime can't tell where idle ends and attack begins
}
\`\`\`

The combined approach forces the slicer into 2x4 (8 frames split as 4+4),
which produces fragile output where 50%+ frames touch cell edges. Animations
also can't be queried by name at runtime — code has to hardcode 'frames 0-3
are idle, frames 4-7 are attack'. The \`animations: { ... }\` object solves
both problems.
...
- **Visual assets go through OGF's two core skills, NEVER raw \`image_gen\`**:
  - \`generate2dsprite\` for any DISCRETE in-game object (character /
    ...
    asset = one skill call. Multi-animation entities (any character
    with idle + walk + attack etc) MUST split into ONE skill call PER
    ANIMATION — never combine into a single 'combat' 8-frame sheet
    (skill picks 2x4, slicer breaks). Catalog uses
    \`animations: { idle: {sprite, fps}, attack: {...} }\` open object,
    not single \`sprite\` field. See full conventions for examples.
